...
*/
		...
		$query = "INSERT INTO domain_records (id_domain_zone, record_type, fqdn, domain, mx_prior, record) VALUES (";
		$query .= "".$id_zone.", ";
		$query .= "'".$type."', ";
...
*/
		...
		$query .= "id_domain_zone=".$id_zone.", ";
		$query .= "fqdn='".$domain.".".$zone_name."', ";
		$query .= "domain='".$domain."', ";
		$query .= "record_type='".$type."', ";
		$query .= "mx_prior=".$prior.", ";
		$query .= "record='".$record."' ";
...
*/
		...
		$query = "DELETE FROM domain_records ";
		$query .= "WHERE id_table=".$id_domain;
	...
	function fetch_detailed_subdomain($id_zone, $domain, $type, $prior, $record) {
		...
		$result = $this->SQL->exec_query("SELECT
			...
			WHERE dr.id_domain_zone = ".$id_zone."
			AND dr.domain='".$domain."'
			AND dr.record_type='".$type."'
			AND dr.mx_prior=".$prior."
			AND dr.record='".$record."'");
	...
	function fetch_zone_info($zone_fqdn) {

		$this->HC->connect();
		$result = $this->HC->info_zone_info($zone_fqdn, "MX");
		$this->HC->disconnect();
	...
	function fetch_zone_plain_text($id_zone) {
		...
		$this->HC->connect();
		$result2 = $this->HC->info_zone($zone_data->fqdn);
		$this->HC->disconnect();
