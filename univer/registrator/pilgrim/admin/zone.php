<?php
	...
	function body_page() {
		...
		if ($id_client == "" || $id_client == "none") {
			...
			$zones = $PM->ZC->fetch_all_zones_of_client($id_client);
			if ($zones == FALSE) {
				echo "Can't fetch all zones: ".$_SESSION["adminpanel_error"];
				return;
			}
		...
		for ($i = 0; $i < $PM->ZC->SQL->get_num_rows($zones); $i++) {
			...
			echo "<tr>
				...
				<td class=\"content_3\"><a href=\"zone_change.php?id=".$data->id_table."\">".$data->fqdn."</a></td>
				<td class=\"content_3\"><a href=\"client_change.php?id=".$data->id_client."\">".$data->client."</a></td>
				<td class=\"content_3\">".$data->type."</td>
				<td class=\"content_3\">0</td>
			</tr>";
			$zones_empty = FALSE;
		}
