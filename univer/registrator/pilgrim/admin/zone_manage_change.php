<?php
	...
	function body_page() {
		...
		$subdomains = $PM->ZC->SDC->fetch_all_subdomains_of_zone($id_zone);
		if ($subdomains == FALSE) {
			echo "Can't fetch all subdomains of zone: ".$_SESSION["adminpanel_error"];
			return;
		}

		$zone_info = $PM->ZC->SDC->fetch_zone_info($zone->fqdn);
		if ($zone_info == FALSE) {
		...
		<script language=javascript>
			...
			function check_form(form_id) {
				...
				if (form_id == \"create_d_mx\") {
					...
					if (new_pri < 0 && new_pri != \"\") {
						alert('Нельзя установить для записи MX приоритет меньше нуля');
						all_right = \"false\";
					}
				...
				<td colspan=\"6\" width=\"100%\">
					...
					<tr valign=\"center\" height=\"62\" width=\"100%\">
						...
						<td height=\"62\" width=\"99%\" align=\"left\" background=\"../img/table/table_background.jpg\" class=\"title\" colspan=\"3\">
							Manage Client's Zone <a class=\"title\" href=\"zone_change.php?id=".$id_zone."\">".$zone->fqdn."</a> of client <a class=\"title\" href=\"client_change.php?id=".$client->id_table."\">".$client->login."</a>
						</td>
		...
		<table cellspacing=\"3\" cellpadding=\"0\" border=\"0\" width=\"100%\">
			<tr valign=\"top\">
				<td colspan=\"6\" width=\"100%\" class=\"content\">Domain Records of Domain Zone <strong>".$zone->fqdn."</strong></td>
			</tr>
			<tr valign=\"top\">
				<td width=\"4%\" class=\"content\">&#035;</td>
				<td width=\"22%\" class=\"content\">Domain</td>
		...
		for ($i = 0; $i < $PM->SDC->SQL->get_num_rows($subdomains); $i++) {
			$data = $PM->SDC->SQL->get_object($subdomains);
			...
			echo "
				<tr valign=\"top\">
					...
					<input type=\"hidden\" name=\"old_d_name\" value=\"".$data->domain."\">
					<input type=\"hidden\" name=\"old_rt\" value=\"".$data->record_type."\">
					<input type=\"hidden\" name=\"old_pri\" value=\"".$data->mx_prior."\">

					<td class=\"dns\">
						".$index++."
					</td>
					<td class=\"dns\">
						<input class=\"simple_input\" type=\"text\" size=\"17\" maxlength=\"255\" name=\"new_d_name\" value=\"".$data->domain."\">
					</td>
					<td class=\"dns\">
						<select class=\"simple_select\" name=\"new_rt\" onChange=\"check_rt('modify_record_".$i."')\">
							<option value=\"A\" ".($data->record_type=="A"?"selected":"").">A</option>
							<option value=\"CNAME\" ".($data->record_type=="CNAME"?"selected":"").">CNAME</option>
							<option value=\"MX\" ".($data->record_type=="MX"?"selected":"").">MX</option>
							<option value=\"NS\" ".($data->record_type=="NS"?"selected":"").">NS</option>
						</select>
					</td>
					<td class=\"dns\">
						<input class=\"simple_input\" type=\"text\" name=\"new_pri\" size=\"4\" value=\"".$data->mx_prior."\">
					</td>
					<td class=\"dns\">
		...
		echo "
			<tr valign=\"top\">
				...
				<td class=\"dns\">
					<input class=\"simple_input\" type=\"text\" name=\"new_rr\" value=\"".$GLOBALS["PRIMARY_IP"]."\" >
				</td>
				<td class=\"dns\">
