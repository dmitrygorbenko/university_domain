...
function safe_get($HTTP_VARS, $param_name, $param_length)
{
	...
	$why_canceled = safe_get($HTTP_POST_VARS, "why", 500);
...
// OK - WORKS
	...
	$refused_time = date("Y-m-d G:i:s", time());
	
	$query = "INSERT INTO dates_table (id_domain_data, query_post_date, confirm_date, lease_time, refused) VALUES ('".$id_domain_data."', '".$query_time."', '".$confirm_time."', '".$lease_time."', '".$refused_time."')";
	print("<br>".$query."<br>");
	$dbresult = $db->simpleQuery($query);
	if (DB::isError($dbresult))
		SQL_error("Can't make query to ".$system_sql_driver.":\n\t\tstring for query: ".$query,
...
// OK - WORKS
	...
	$query = "INSERT INTO descr_table (id_domain_data, add_info, why_released, why_refused, notes) VALUES ('".$id_domain_data."', '".$add_info."', '', '".$why_canceled."', '')";
	print("<br>".$query."<br>");
	$dbresult = $db->simpleQuery($query);
	if (DB::isError($dbresult))
		SQL_error("Can't make query to ".$system_sql_driver.":\n\t\tstring for query: ".$query,
...
// OK - WORKS
	...
	$query = "INSERT INTO domain_status_table (id_domain_data, wait_for_pay, refused) VALUES ('".$id_domain_data."', 0, 1)";
	print("<br>".$query."<br>");
	$dbresult = $db->simpleQuery($query);
	if (DB::isError($dbresult))
		SQL_error("Can't make query to ".$system_sql_driver.":\n\t\tstring for query: ".$query,
...
// OK - WORKS
	...
	$query = "INSERT INTO refused (id_domain_data) VALUES ('".$id_domain_data."')";
	print("<br>".$query."<br>");
	$dbresult = $db->simpleQuery($query);
	if (DB::isError($dbresult))
		SQL_error("Can't make query to ".$system_sql_driver.":\n\t\tstring for query: ".$query,
	...
	$mail_to = $email_addr;
	$subject = "Регистрация доменного имени ".$domain_name.".".$domain_zone.": Отклонено";
	$text = "
	Здравствуйте, ".$first_name." ".$last_name." 
	
	Ваша заявка на регистрацию не была принята.
	Домен к регистрации:
	...
	Если Вы не согласны с этим - напишите об этом
	на адрес: ".$system_offence_email."
