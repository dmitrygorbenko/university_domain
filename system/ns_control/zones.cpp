...
unsigned int create_new_zone(char * cmd)
{
	...
	// Fill up zone config
	zf = fopen(config_data.named_zone_file_list, "a");
...
zone_file_name);
	...
	// Fill up zone file
	zone_file_name = merge_strings(3, config_data.named_zone_dir, zone_name, ".zone");
...
zone_name,
zone_name,
config_data.soa_email_addr,
serial->serial_num,
config_data.ns_server,
config_data.ns2_server,
config_data.mx_server,
config_data.mx2_server,
config_data.primary_ip,
zone_name);
...
unsigned int update_zone(char * cmd)
{
	...
	if (!zone_name) {
		result = FALSE;
		message("Failed on get zone_name\n");
	...
	if (!new_zone_name) {
		result = FALSE;
		message("Failed on get new_zone_name\n");
		goto update_zone_end;
	}
	...
	zone_list_file_name = strdup(config_data.named_zone_file_list);
	zone_file_name = merge_strings(3, config_data.named_zone_dir, zone_name, ".zone");
	new_zone_file_name = merge_strings(3, config_data.named_zone_dir, new_zone_name, ".zone");
	short_new_zone_file_name = merge_strings(2, new_zone_name, ".zone");
	tmp_file_name = merge_strings(2, config_data.named_zone_dir, unique);

	// UPDATE ZONE FILE LIST

	zf = fopen(zone_list_file_name, "r");
	if (!zf) {
		message("Failed to open zone_file\n");
		result = FALSE;
		goto update_zone_end;
	}
	...
	if (!tmpf) {
		fclose(zf);
		message("Failed to create temporary_file\n");
		result = FALSE;
		goto update_zone_end;
	}
...
short_new_zone_file_name);
	...
	if (rename(tmp_file_name, zone_list_file_name) == -1) {
		message("Can not rename file %s to %s\n", tmp_file_name, zone_list_file_name);
		result = FALSE;
		goto update_zone_end;
	}

	if (zone_list_update == FALSE) {
		message("Zone file list not updated !\n");
		result = FALSE;
		goto update_zone_end;
	}
...
new_zone_name);
	...
	if (unlink(zone_file_name) == -1) {
		message("Can not remove file: %s\n", zone_file_name);
	}

	if (rename(tmp_file_name, new_zone_file_name) == -1) {
		message("Can not rename file %s to %s\n", tmp_file_name, new_zone_file_name);
		result = FALSE;
		goto update_zone_end;
	}
...
unsigned int remove_zone(char * cmd)
{
	...
	zone_file_name = strdup(config_data.named_zone_file_list);
