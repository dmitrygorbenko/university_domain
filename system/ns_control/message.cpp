...
void message(char *format, ...)
{
	...
	va_end(ap);
	msg = safe_strdup(buffer);
	msg = merge_strings(5, process_id, ": ", date, ": ", msg);
...
void p_error(char *format, ...)
{
	...
	va_end(ap);
	error = safe_strdup(buffer);
	error = merge_strings(5, process_id, ": ", date, ": ", error);
...
void __message(char * msg_string)
{
	...
	if (m_state.log_init == TRUE) {
		...
		if (m_state.log_fd == NULL) {
			p_error("Message(): Can't open/create log file (%s)\n", m_state.log_file_name);
			return;
		}
...
void __p_error(char * err_string)
{
	...
	if (m_state.log_init == TRUE) {
		...
		if (m_state.log_fd == NULL) {
			p_error("__p_error(): Can't open/create log file (%s)\n", m_state.log_file_name);
			return;
		}
