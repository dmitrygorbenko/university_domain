...
//quake.hosting.ai                IN SOA  quake.hosting.ai root.deep.lan. (
//                                2005080600 ; serial
...
//  1   2     3         4       5       6       7         7 or 8       8 or 9
	...
	if (!zone_name || !type) {
		result = FALSE;
		message("Failed on check zone_name, type\n");
		goto update_zone_info_end;
	}

	if (strcmp(type, "MX") != 0) {
		result = FALSE;
		message("Failed on type check\n");
		goto update_zone_info_end;
	}
	...
	if (!new_type) {
		result = FALSE;
		message("Failed on check and type\n");
		goto update_zone_info_end;
	}

	if (strcmp(new_type, "MX") != 0) {
		result = FALSE;
		message("Failed on new_type\n");
		goto update_zone_info_end;
	}
	...
	if (strcmp(new_type, "MX") == 0) {
		...
		if (!new_prior) {
			result = FALSE;
			message("Failed on get new_prior\n");
			goto update_zone_info_end;
		}
	}
	...
	if (!new_record) {
		result = FALSE;
		message("Failed on get new_record\n");
		goto update_zone_info_end;
	}
	...
	if (update_zone_serial(zone_name) == FALSE) {
		result = FALSE;
		message("Failed on update serial\n");
		goto update_zone_info_end;
	}
	...
	while (!feof(zf)) {
		...
		if (strcmp(tmp, "IN") == 0) {
			...
			if (strcmp(tmp2, type) == 0) {
				...
				if (!tmp3 || !tmp4) {
					result = FALSE;
					message("Failed to get tmp3 and tmp4\n");
					goto update_zone_info_end;
				}
...
unsigned int update_zone_manual(char * cmd)
{
	...
	if (!zone_name || !zone_content) {
		result = FALSE;
		message("Failed on check zone_name, zone_content\n");
...
unsigned int remove_zone_info(char * cmd)
{
	...
	if (!zone_name || !type) {
		message("Failed to zone and type check\n");
		result = FALSE;
